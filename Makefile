...
help:
	@echo "Flux (C++) - Available Commands"
	@echo "================================"
	@echo ""
	@echo "Setup:"
	...
	@echo "  make install           Build backend (CMake + vcpkg)"
...
check-prereqs:
	...
	@command -v cmake >/dev/null 2>&1 || { echo "cmake is required but not installed. Visit https://cmake.org"; exit 1; }
	@command -v make >/dev/null 2>&1 || { echo "make is required but not installed."; exit 1; }
	@if [ -z "$$VCPKG_ROOT" ]; then echo "VCPKG_ROOT is not set. Visit https://vcpkg.io"; exit 1; fi
...
init: check-prereqs
	...
	@echo ""
	@echo "==> Building C++ backend (CMake + vcpkg)..."
