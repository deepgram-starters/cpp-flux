...
C++ (Crow) demo app for Deepgram Flux.

## Architecture

- **Backend:** C++ (Crow) (C++) on port 8081
...
| File | Purpose |
|------|---------|
| `src/main.cpp` | Main backend — API endpoints and WebSocket proxy |
...
**Clean rebuild:**
```bash
rm -rf build frontend/node_modules frontend/.vite
make init
```

## Dependencies

- **Backend:** `CMakeLists.txt + vcpkg.json` — Uses vcpkg for C++ deps and CMake for builds. Set VCPKG_ROOT env var. Crow framework for HTTP/WebSocket.
- **Frontend:** `frontend/package.json` — Vite dev server
- **Submodules:** `frontend/` (flux-html), `contracts/` (starter-contracts)

Install: `cmake -B build -S . -DCMAKE_TOOLCHAIN_FILE="$VCPKG_ROOT/scripts/buildsystems/vcpkg.cmake" && cmake --build build`
...
4. Handle it in the backend `src/main.cpp` — read the param and pass it to the Deepgram API
...
```
feat(cpp-flux): add diarization support
fix(cpp-flux): resolve WebSocket close handling
refactor(cpp-flux): simplify session endpoint
